...
The files generated by SUSHI are located in the _FishExample/build_ directory. Check that the _build/input/profiles_ subdirectory contains two generated structure definitions.
...
Since fish don't get legally married (although some species do pair bond) and they don't communicate in a human language, the first thing we'll do in the FishPatient profile is eliminate these elements. To do this, open the file _FishPatient.fsh_ in your favorite plain-text editor, and add the following rules after `Description` line:
...
Run SUSHI again. The count of ValueSets should now be 1. Try generating the IG by changing directories to _build_ and running `_genonce` again. Open the file `/FishExample/build/output/index.html` to see the resulting IG.
...
If you need help with this, you can reference the FSH files in the _FishExampleComplete_ directory.
...
Congratulations! You've completed the FSH tutorial. It might be time to dine on some sushi!